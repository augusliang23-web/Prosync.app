...
export const LinkedInDMShowcaseModal: React.FC<LinkedInDMShowcaseModalProps> = ({
  ...
  const linkedinCopies: Record<TabType, string> = {
...
#ProjectManager #Agile #WorkplaceProductivity #PMTools #週報自動化`
  ...
  const handleCopyText = () => {
    navigator.clipboard.writeText(linkedinCopies[activeTab]);
    setCopied(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[92vh] shadow-2xl flex flex-col border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-base tracking-tight text-white flex items-center gap-2">
                <span>LinkedIn 宣傳 DM 與介面圖卡生成中心</span>
              ...
              <p className="text-xs text-slate-300 mt-0.5">具備底部漸層融雪消失效果 (Gradient Fade-Out) 的精選產品介面圖卡，專供 LinkedIn 貼文與推廣 DM</p>
            </div>
          </div>
        ...
        <div className="bg-slate-100/90 p-2 border-b border-slate-200 flex items-center gap-1.5 overflow-x-auto shrink-0">
          {[
            { id: 'DASHBOARD', label: '1. 高階 AI 戰情室', icon: LayoutDashboard },
            { id: 'TREND', label: '2. 12~24週推移圖表', icon: TrendingUp },
            { id: 'APPROVAL', label: '3. N-1 主管簽核關卡', icon: FileCheck2 },
            { id: 'PM_EDITOR', label: '4. PM 週報與編輯器', icon: FolderGit2 },
          ].map((tab) => {
            const Icon = tab.icon;
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
                        <div className="text-[10px] text-slate-400 uppercase font-bold">全公司總預算</div>
                        <div className="text-base font-extrabold text-slate-900 font-mono mt-1">NT$ 128,500,000</div>
                        <div className="text-[10px] text-emerald-600 font-medium mt-0.5">多幣別自動清算換算</div>
                      </div>
                      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
                        <div className="text-[10px] text-slate-400 uppercase font-bold">預算支用率</div>
                        <div className="text-base font-extrabold text-indigo-600 font-mono mt-1">42.8 %</div>
                        <div className="text-[10px] text-slate-500 font-medium mt-0.5">NT$ 55,000,000 已列支</div>
                      </div>
                      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
                        <div className="text-[10px] text-slate-400 uppercase font-bold">健康狀態佈局</div>
                        <div className="flex items-center gap-1.5 mt-1 font-bold text-xs">
                          <span className="text-emerald-600">🟢 3 正常</span>
                          <span className="text-amber-600">🟡 2 注意</span>
                          <span className="text-rose-600">🔴 1 延宕</span>
                        </div>
                      </div>
                      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
                        <div className="text-[10px] text-slate-400 uppercase font-bold">平均推進進度</div>
                        <div className="text-base font-extrabold text-emerald-600 font-mono mt-1">68.5 %</div>
                        <div className="text-[10px] text-slate-500 font-medium mt-0.5">預計完工率穩健</div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-4 rounded-xl shadow-md space-y-2">
                      <div className="flex items-center justify-between border-b border-indigo-700/50 pb-2">
                        <span className="text-xs font-extrabold flex items-center gap-1.5 text-indigo-200">
                          <Sparkles className="w-4 h-4 text-amber-300" /> Executive Briefing Memo (C-Suite AI 戰略備忘錄)
                        </span>
                        <span className="text-[10px] font-mono text-indigo-300">W24 最新週報彙整</span>
                      </div>
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-2">
                      <div className="text-xs font-bold text-slate-700">部門專案脈搏 (Department Pulse)</div>
                      <div className="grid grid-cols-3 gap-2 text-xs font-medium">
                        <div className="p-2 rounded bg-slate-50 border border-slate-200">研發部：🟢 2 專案進行中</div>
                        <div className="p-2 rounded bg-slate-50 border border-slate-200">IT資訊部：🟡 1 專案風險</div>
                        <div className="p-2 rounded bg-slate-50 border border-slate-200">永續部：🟢 1 專案進行中</div>
                      </div>
                    </div>
                  </div>
                )}
                    ...
                    <div className="bg-amber-50 border border-amber-300/80 rounded-2xl p-4 shadow-2xs space-y-3">
                      <div className="flex items-center justify-between pb-2 border-b border-amber-200">
                        <h3 className="text-xs font-bold text-amber-950 flex items-center gap-2">
                          ...
                          <span>部門 N-1 / 高層簽核關卡：待核准專案變更 (Approval Gateway)</span>
                        </h3>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-white p-3 rounded-xl border border-amber-200 space-y-1.5 text-xs">
                          <div className="font-bold text-slate-900">新一代 AI 智慧物流分揀系統 (PRJ-2026-001)</div>
                          <div className="text-[11px] text-amber-700 font-mono">展延申請：2026-09-30 &rarr; 2026-10-15</div>
                          <div className="text-[11px] text-slate-600">變更理由：等待第三代感測器軟體 SDK 更新測試</div>
                          <div className="pt-2 flex justify-end gap-1.5">
                            <span className="px-2.5 py-1 rounded bg-slate-100 text-slate-600 text-[10px] font-bold">退回申請</span>
                            <span className="px-2.5 py-1 rounded bg-emerald-600 text-white text-[10px] font-bold">核准並更新基線</span>
                          </div>
                        </div>

                        <div className="bg-white p-3 rounded-xl border border-amber-200 space-y-1.5 text-xs">
                          <div className="font-bold text-slate-900">綠色供應鏈範疇三碳盤查平台 (PRJ-2026-005)</div>
                          <div className="text-[11px] text-indigo-700 font-mono">新增里程碑：完成歐盟 CSRD 規範查核認證</div>
                          <div className="text-[11px] text-slate-600">預定完成日：2026-11-20</div>
                          <div className="pt-2 flex justify-end gap-1.5">
                            <span className="px-2.5 py-1 rounded bg-slate-100 text-slate-600 text-[10px] font-bold">退回申請</span>
                            <span className="px-2.5 py-1 rounded bg-emerald-600 text-white text-[10px] font-bold">核准並更新基線</span>
                          </div>
                  ...
                  <div className="space-y-3">
                    <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-3">
                      <div className="flex items-center justify-between border-b pb-2">
                        <div className="font-bold text-xs text-slate-900">專案經理編輯模組 (PM Project Editor & Weekly Log)</div>
                      ...
                      <div className="grid grid-cols-3 gap-3 text-xs">
                        <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
                          <span className="text-[10px] font-bold text-slate-500">專案名稱</span>
                          <p className="font-bold text-slate-900">智慧醫療影像輔助診斷系統</p>
                        </div>
                        <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
                          <span className="text-[10px] font-bold text-slate-500">總預算與計價幣別</span>
                          <p className="font-mono font-bold text-slate-900">USD $ 850,000 (TWD $ 27,200,000)</p>
                        </div>
                        <div className="p-2.5 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
                          <span className="text-[10px] font-bold text-slate-500">完成進度</span>
                      ...
                      <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200 space-y-2 text-xs">
                        <div className="font-bold text-slate-700">核心里程碑交付清單 (Key Deliverables)</div>
                        <div className="space-y-1">
                          <div className="flex items-center justify-between bg-white p-1.5 rounded border border-slate-200">
                            <span className="font-semibold text-slate-800">☑️ 完成臨床數據 TFDA 稽核備查</span>
                            <span className="font-mono text-[10px] text-slate-500">2026-07-15</span>
                          </div>
                          <div className="flex items-center justify-between bg-white p-1.5 rounded border border-slate-200">
                            <span className="font-semibold text-slate-800">☐ 第一階段院區試行場域上線部署</span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-extrabold text-slate-900 text-xs">LinkedIn 建議貼文內文 (LinkedIn Copy Template)</h4>
                  <p className="text-[11px] text-slate-500">一鍵複製下方文案，即可直接貼至 LinkedIn 進行行銷推廣</p>
                </div>
              </div>
          ...
          <div className="bg-indigo-50/80 rounded-xl p-3.5 border border-indigo-200/80 text-xs text-indigo-900 flex items-start gap-2.5">
            ...
            <div>
              <span className="font-bold block text-indigo-950">📸 如何完美發佈至 LinkedIn：</span>
              <p className="text-[11px] text-indigo-800/90 leading-normal mt-0.5">
                使用作業系統快捷鍵（Mac: <kbd className="px-1 py-0.5 bg-white rounded border border-indigo-300 font-mono text-[10px]">Cmd+Shift+4</kbd> / Windows: <kbd className="px-1 py-0.5 bg-white rounded border border-indigo-300 font-mono text-[10px]">Win+Shift+S</kbd>）框選上方上方含漸層融雪效果的圖卡框，再點選「一鍵複製 LinkedIn 貼文內文」，即可快速在 LinkedIn 建立吸睛貼文！
              </p>
            </div>
          </div>
